...
export class CustomerCategoryService {
  ...
  async findAll(includeDeleted: boolean = false): Promise<CustomerCategoryResponseDto[]> {
  ...
  async findOne(id: string, includeDeleted: boolean = false): Promise<CustomerCategoryResponseDto> {
  ...
  async findByName(name: string, includeDeleted: boolean = false): Promise<CustomerCategoryResponseDto> {
    const category = await this.customerCategoryRepository.findOne({
      where: { name },
      withDeleted: includeDeleted,
    });
  ...
  async remove(id: string): Promise<{ message: string }> {
    ...
    if (category.customers && category.customers.length > 0) {
      throw new ConflictException('No se puede eliminar la categoría porque tiene clientes asociados');
    }
